...
vi.mock('three', () => ({
  ...
  Mesh: vi.fn(function(this: any, geo: any, mat: any) {
    ...
    this.material = mat || { emissiveIntensity: 0.5, dispose: vi.fn() };
...
describe('Vortex Generator', () => {
  it('registers itself in StructureRegistry', () => {
    expect(StructureRegistry.get('vortex')).toBeDefined();
  });

  it('returns bounding radius > 0', () => {
    const gen = StructureRegistry.get('vortex')!;
  ...
  it('has component presence (group defined)', () => {
    const gen = StructureRegistry.get('vortex')!;
    const result = gen(42, 5, color);
