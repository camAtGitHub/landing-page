...
describe('Mushroom Generator', () => {
  it('registers itself in StructureRegistry', () => { expect(StructureRegistry.get('mushroom')).toBeDefined(); });
  it('returns bounding radius > 0', () => { expect(StructureRegistry.get('mushroom')!(42,5,color).boundingRadius).toBeGreaterThan(0); });
  it('produces deterministic output for same seed', () => { const g=StructureRegistry.get('mushroom')!; expect(g(42,5,color).boundingRadius).toBe(g(42,5,color).boundingRadius); });
  it('produces valid output for different seeds', () => { const g=StructureRegistry.get('mushroom')!; [1,100,999].forEach(s => expect(g(s,3,color).boundingRadius).toBeGreaterThan(0)); });
  it('update() does not throw', () => { const r=StructureRegistry.get('mushroom')!(42,5,color); expect(()=>r.update(1.0,0.016)).not.toThrow(); });
  it('dispose() does not throw', () => { const r=StructureRegistry.get('mushroom')!(42,5,color); expect(()=>r.dispose()).not.toThrow(); });
});
