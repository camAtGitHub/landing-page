...
describe('Entity Generator', () => {
  it('registers itself in StructureRegistry', () => { expect(StructureRegistry.get('entity')).toBeDefined(); });
  it('returns bounding radius > 0', () => { expect(StructureRegistry.get('entity')!(42,5,color).boundingRadius).toBeGreaterThan(0); });
  it('is deterministic for same seed', () => { const g=StructureRegistry.get('entity')!; expect(g(42,5,color).boundingRadius).toBe(g(42,5,color).boundingRadius); });
  it('group has children (complex structure)', () => { const r=StructureRegistry.get('entity')!(42,5,color); expect(r.group.children.length).toBeGreaterThan(10); });
  it('update() does not throw', () => { const r=StructureRegistry.get('entity')!(42,5,color); expect(()=>r.update(1.0,0.016)).not.toThrow(); });
  it('dispose() does not throw', () => { const r=StructureRegistry.get('entity')!(42,5,color); expect(()=>r.dispose()).not.toThrow(); });
});
