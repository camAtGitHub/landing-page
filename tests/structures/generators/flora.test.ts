...
vi.mock('three', () => ({
  ...
  BufferGeometry: vi.fn(() => ({ dispose: vi.fn(), setFromPoints: vi.fn() })),
}));
...
describe('Flora Generator', () => {
  it('registers itself in StructureRegistry', () => {
    expect(StructureRegistry.get('flora')).toBeDefined();
  });

  it('returns bounding radius > 0', () => {
    const gen = StructureRegistry.get('flora')!;
    const result = gen(42, 5, color);
    expect(result.boundingRadius).toBeGreaterThan(0);
  });

  it('sub-variant is consistent for same seed', () => {
    const gen = StructureRegistry.get('flora')!;
  ...
  it('different seeds produce valid bounding radii', () => {
    const gen = StructureRegistry.get('flora')!;
    const r1 = gen(42, 5, color);
